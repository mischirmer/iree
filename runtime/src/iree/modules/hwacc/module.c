...
IREE_VM_ABI_FIXED_STRUCT(rrrIIIff, {
  iree_vm_ref_t r0;
  iree_vm_ref_t r1;
  iree_vm_ref_t r2;
  int64_t i3;
...
IREE_VM_ABI_DEFINE_SHIM(rrrIIIff, r);
...
IREE_VM_ABI_EXPORT(iree_hwacc_module_hwacc_gemm_f32, iree_hwacc_module_state_t,
  ...
  // Expect HAL buffer views for tensor values (not VM buffers).
  ...
  // Accept a possibly-null output buffer view reference.
  ...
  // Map input buffer_views' underlying buffers for read access.
  iree_hal_buffer_t* a_buf = iree_hal_buffer_view_buffer(a_view);
  iree_hal_buffer_t* b_buf = iree_hal_buffer_view_buffer(b_view);
  iree_hal_buffer_mapping_t a_mapping;
  iree_hal_buffer_mapping_t b_mapping;
  IREE_RETURN_IF_ERROR(iree_hal_buffer_map_range(
    a_buf, IREE_HAL_MAPPING_MODE_SCOPED, IREE_HAL_MEMORY_ACCESS_READ, 0,
    iree_hal_buffer_byte_length(a_buf), &a_mapping));
  IREE_RETURN_IF_ERROR(iree_hal_buffer_map_range(
    b_buf, IREE_HAL_MAPPING_MODE_SCOPED, IREE_HAL_MEMORY_ACCESS_READ, 0,
    iree_hal_buffer_byte_length(b_buf), &b_mapping));

  const float* A = (const float*)a_mapping.contents.data;
  const float* B = (const float*)b_mapping.contents.data;

  // If the caller provided an output buffer, map and use it. Otherwise,
  // allocate a VM buffer of the required size and use that.
  /* out_span removed: we map into iree_hal_buffer_mapping_t for outputs. */
  ...
  if (c_view) {
    // Use provided buffer_view: map its underlying buffer for RW and write
    // results into it. Then return a retained reference to the provided view.
    iree_hal_buffer_t* out_hal_buf = iree_hal_buffer_view_buffer(c_view);
    iree_hal_buffer_mapping_t out_mapping;
    IREE_RETURN_IF_ERROR(iree_hal_buffer_map_range(
        out_hal_buf, IREE_HAL_MAPPING_MODE_SCOPED, IREE_HAL_MEMORY_ACCESS_ALL,
        0, iree_hal_buffer_byte_length(out_hal_buf), &out_mapping));

    float* C = (float*)out_mapping.contents.data;

    // Call into existing runtime implementation which writes into C.
    hwacc_gemm_f32(A, B, C, M, N, K, alpha, beta);

    // Unmap the output buffer.
    IREE_RETURN_IF_ERROR(iree_hal_buffer_unmap_range(&out_mapping));

    // Return the provided buffer_view: retain then move the retained pointer
    // into the VM ref that we return.
    iree_hal_buffer_view_retain(c_view);
    rets->r0 = iree_hal_buffer_view_move_ref(c_view);
  } else {
    // Allocate a HAL heap allocator (host-local) and use it to create a
    // hal buffer+buffer_view to return as the tensor result. This makes the
    // returned ref have the same type the compiled module expects
  ...
  iree_hal_buffer_params_t params = {0};
    // Request host-local memory that is mappable and dense.
  ...
  // Note: allocate_buffer takes params by value (not pointer).
  IREE_RETURN_IF_ERROR(iree_hal_allocator_allocate_buffer(
    heap_allocator, params, (iree_device_size_t)byte_length, &hal_buffer));

    // Map the hal buffer for write and copy results into it.
    iree_hal_buffer_mapping_t mapping;
    IREE_RETURN_IF_ERROR(iree_hal_buffer_map_range(
        hal_buffer, IREE_HAL_MAPPING_MODE_SCOPED, IREE_HAL_MEMORY_ACCESS_ALL,
        0, byte_length, &mapping));
  ...
  // Unmap the buffer (scoped mapping will be unmapped when mapping goes out
  // of scope, but we call unmap to be explicit).
  IREE_RETURN_IF_ERROR(iree_hal_buffer_unmap_range(&mapping));

    // Create a buffer_view wrapping the allocated buffer and return it.
    iree_hal_buffer_view_t* buffer_view = NULL;
    iree_hal_dim_t shape[2];
    shape[0] = (iree_hal_dim_t)M;
    shape[1] = (iree_hal_dim_t)N;
    IREE_RETURN_IF_ERROR(iree_hal_buffer_view_create(
        hal_buffer, /*shape_rank=*/2, shape,
    ...
    // Release our local reference to the buffer (buffer_view retains it).
    iree_hal_buffer_release(hal_buffer);
    // Release the allocator; buffer and buffer_view retain whatever they need.
    iree_hal_allocator_release(heap_allocator);

    rets->r0 = iree_hal_buffer_view_move_ref(buffer_view);
  }

  // Unmap input mappings now that work is complete.
  ...
  // If either unmap failed prefer to return that status.
